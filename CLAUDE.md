...
3. **Markdown 表格上方必须加 `<!-- prettier-ignore -->` 注释**（新建和修改时都要有）。理由：prettier 会用填充空格重新对齐表格，导致 AI 编辑工具的精确文本匹配反复失败。现有表格已全部加注释，今后新表格遗漏此注释视为格式违规。
...
> **[强制工具调用] 遇到构建失败、缺库、依赖/打包类问题时，第一步先读 `cat docs/运行与打包指南.md`（重点看「常见坑」表），确认没有已知解法再动手排查。严禁不查文档直接 grep 源码或 node_modules。**
...
| `docs/运行与打包指南.md` | 修改网络请求/服务地址/打包配置/**依赖**之前；排查构建/缺库/依赖问题时的第一入口 |
