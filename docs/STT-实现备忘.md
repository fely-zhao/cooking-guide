...
> 2026-09-02 审计 B2：ask 词条已随提问入口临时禁用（词表注释保留于 voice-commands.ts，
> InteractionControls 的 ASK_FEATURE_ENABLED=false）。禁用期间含「我想问」的语句要么命中
> next/repeat，要么无命中被忽略。提问功能设计完成后恢复。
