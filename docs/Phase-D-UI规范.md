...
- `RecipeDetailScreen.tsx` / `Button.tsx`：hero 返回按钮 label、份数 stepper 符号按钮 label、备料食材卡 `checkbox` 角色 + `checked`/`disabled` 语义；`Button` 组件新增 accessibilityLabel 透传（title 为符号时使用）（a11y 第五轮，2026-09-05，全屏覆盖收官）
