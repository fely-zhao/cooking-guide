...
| 2    | UI 文案抽取：38 文件硬编码替换为 t('key')，分屏幕推进；FSM 层「烹饪完成」两处改 i18n.t()                            | grep 无硬编码 UI 文案（排除注释），每屏切语言即时生效    | 代码完成：10 屏 + 10 组件 + FSM/服务层已抽取，四连全过，grep 验收通过（排除注释/prompt/语言名）；待真机逐屏验收 |
